    ...
    return (
      <div className="winners">
        <div className="fame-wait">
          ...
          played. Until then it's all still to play for — keep making your picks.</p>
        </div>
        <style>{`
...
// --------------------------------------------------------------- Reviews
function Reviews({ session, meName }) {
  const [review, setReview] = useState(undefined); // undefined = loading
  const [finalDone, setFinalDone] = useState(null);
  useEffect(() => {
    (async () => {
      const [{ data: r }, { data: fin }] = await Promise.all([
        supabase.from("reviews").select("review").eq("user_id", session.user.id).maybeSingle(),
        supabase.from("fixtures").select("finished").eq("stage", "FINAL").eq("finished", true).limit(1),
      ]);
  ...
  if (review === undefined || finalDone === null) return <p className="muted center">Loading…</p>;

  const styleTag = (
    <style>{`
      .review-wait { text-align:center; padding:3rem 1.5rem; border:1px solid var(--line);
        border-radius:18px; background:var(--panel); backdrop-filter:blur(3px); }
      .review-emoji { font-size:3rem; margin-bottom:0.4rem; }
      .review-wait h1 { font-size:1.7rem; margin:0.3rem 0 0.6rem; }
      .review-card { border:1px solid var(--gold); border-radius:18px; padding:1.6rem 1.4rem;
        background:var(--panel); backdrop-filter:blur(3px); }
      .review-badge { color:var(--gold); letter-spacing:0.2em; font-size:0.7rem; font-weight:700; }
      .review-card h1 { font-size:1.9rem; margin:0.3rem 0 0.9rem; }
      .review-text { white-space:pre-wrap; line-height:1.65; font-size:1.02rem; }
    `}</style>
  );
    ...
    return (
      <div className="winners">
        ...
        <div className="review-wait">
          ...
          <h1 className="display">Your review is coming</h1>
          <p className="muted">Come back here after the final to read a funny, honest review of your performance. (Generated by Claude Ai)</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="winners">
      ...
      <div className="review-card">
        <div className="review-badge">YOUR TOURNAMENT, REVIEWED</div>
        <h1 className="display">{cleanName(meName)}…</h1>
    ...
    <>
      ...
      <div className="app">
        ...
        <main className="content">
          ...
          {tab === "winners" && <Winners />}
          {tab === "reviews" && <Reviews session={session} meName={name} />}
